...
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">{{ __('outlet.detail') }}- :)  Puede agregar fotos a su denuncia mas abajo</div>
            <div class="card-body">
                <table class="table table-sm">
                    <tbody>
                        <tr><td><i class="fa fa-bug text-primary" aria-hidden="true"></i> {{ __('outlet.name') }}</td><td>{{ $outlet->name }}</td></tr>
                         <tr><td><i class="fa fa-commenting-o text-primary" aria-hidden="true"></i> Observacion</td><td>{{ $outlet->observacion }}</td></tr>
                       
                        
                        <tr><td><i class="fa fa-list-alt text-primary" aria-hidden="true"></i> {{ __('outlet.address') }}</td><td>{{ $outlet->address }}</td></tr>
                        ...
                        <tr><td><i class="fa fa-envelope" aria-hidden="true"></i> Email </td><td>{{ $outlet->email }}</td></tr>
                         <tr style="color:#227dc7;"><td ><i class="fa fa-camera" aria-hidden="true"></i> Puede agregar fotos  </td><td>a su denuncia mas abajo<i class="fa fa-picture-o" aria-hidden="true"></i></td></tr>
            ...
            @if (count($imagenes) > 0)
                <div class="alert alert-success">
                    <ul>
                        ...
                        <img src="{{ $imagen->image_path }}" alt="{{ $imagen->name }}" width="200" height="200">
                        @endforeach
                    </ul>
                </div>
            @endif
...
<div class="container mt-3">
        <h3 class="text-center mb-3" style="color:#00FF00;"><i class="fa fa-camera" aria-hidden="true"></i> Agregar Fotos <i class="fa fa-picture-o" aria-hidden="true"></i></h3>
        <form action="{{route('imageUpload')}}" method="post" enctype="multipart/form-data">
            ...
            <button type="submit" name="submit" class="btn btn-primary btn-block mt-2">
               Click para Subir las imágenes y adjuntar a la denuncia
            </button>
        </form>
    </div>
